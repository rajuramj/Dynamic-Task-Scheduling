                ...
                for(;;)
                {
                    ...
                    else
                    {
...

                      {
                              ...
                              if(numTasksDone == Utility::numTasks * Utility::maxIter ){
                            	  stop = true;
                      ...
                      /*{
                      std::lock_guard <std::mutex> locker(Utility::mu);
                      std::cout << " Thread "  << i << " will raise notify all() signal" << std::endl;
                      }*/
...
////          std::forward<F>(f));
//    auto task = std::make_shared< std::packaged_task<bool()> >(f);
//
...
//            throw std::runtime_error("enqueue on stopped ThreadPool");
//
//        tasks.emplace([task]()->bool{ return (*task)(); /*return task->get_future()->get();  });
//    }
//
//    condition.notify_one();
//
